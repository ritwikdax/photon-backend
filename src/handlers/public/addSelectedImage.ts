import { Request, Response } from "express";
import { getDb } from "../../database.js";
import { COLLECTIONS } from "../../const.js";
import { addImagebodySchema } from "./schemas/addImageSchema.js";
import { v4 } from "uuid";

export async function addSelectedImagehandler(req: Request, res: Response) {
  try {
    const now = new Date();
    const projectId = res.locals["projectId"];
    const merchantId = res.locals["merchantId"];

    const body = req.body;
    const parsedBody = addImagebodySchema.parse(body);

    const db = await getDb(merchantId);
    const result = await db
      .collection(COLLECTIONS.IMAGE_SELECTIONS)
      .findOne({ projectId: projectId });

    if (!result || !result.isSelectionAllowed) {
      return res
        .status(403)
        .json({ error: true, message: "Image selection not allowed" });
    }
    await db.collection(COLLECTIONS.SELECTED_IMAGES).insertOne({
      ...parsedBody,
      projectId: projectId,
      id: v4(),
      createdAt: now,
      updatedAt: now,
    });

    return res
      .status(201)
      .json({ message: "Image selection added successfully" });
  } catch (err: any) {
    console.error(err.message);
    return res.status(500).send("Error fetching selected images");
  }
}
